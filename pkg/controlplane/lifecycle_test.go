...
func newLifecycleFixture(t *testing.T, f *trustFixture) *lifecycleFixture {
	...
	lf.cfg = EnrollmentConfig{
		...
		Now:           func() time.Time { return lf.clock },
	}
	return lf
}
...
// it re-enrolls; past expiry it re-enrolls after logging an error.
func TestEnrollmentPasses(t *testing.T) {
	...
	lf.clock = lf.clock.Add(trust.ASValidity - trust.ChainRenewalThreshold + time.Minute)
	...
	chain, err := trust.NewestChain(ctx, lf.db, nodeIATest, lf.clock)
	...
	lf.clock = lf.clock.Add(3 * trust.ASValidity)
	if got := lf.cfg.enrollPass(ctx); got != time.Millisecond {
...
func TestEnrollmentPassFailing(t *testing.T) {
	...
	lf.clock = lf.clock.Add(2 * trust.ASValidity)
...
func TestCoreEnrollmentPass(t *testing.T) {
	...
	chain, err := trust.NewestChain(context.Background(), lf.db, nodeIATest, lf.clock)
	if err != nil || chain == nil {
	...
	fresh, err := trust.NewestChain(context.Background(), lf.db, nodeIATest, lf.clock)
